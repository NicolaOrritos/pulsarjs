...
function savePulse(pulse, interval)
{
    if (pulse)
    {
...
function pulsar()
{
     ...
     *  PUBLIC INTERFACE
     *********************/
    this.loadPulses = function(loaded)
    {
        var self = this;
        
        db.find({}, function(error, pulses)
        {
            if (error)
            {
            ...
            else if (pulses)
            {
                ...
                for (var a=0; a<pulses.length; a++)
                {
                    ...
                    if (pulse.callback)
                    {
                        ...
                        pulsesIDs[a] = self.addPulse(pulse.interval, pulseCallback, pulse.active);
                        
                        console.log("Added pulse with ID '%s'", pulsesIDs[a]);
                    }
                }
            }
            else
            ...
            if (loaded)
            {
                loaded.call(this, pulsesIDs);
            }
        });
    }
    
    this.addPulse = function(interval, callback, id, startNow, parameters)
    {
        if (interval)
        if (callback)
        {
            ...
            console.log("Resulting arguments:");
