...
function pulsar()
{
     ...
     *  PUBLIC INTERFACE
     **********************/
    this.clearPulses = function(callback)
    {
        db.loadDatabase(function(loadError)
        {
            if (loadError)
            ...
            else
            {
                ...
                db.remove({"type": PULSE_TYPE}, {multi: true}, function(err, numRemoved)
                {
                    db.persistence.compactDatafile();
                    
                    callback.call(this, err, numRemoved);
                });
            }
        });
    };
    ...
    this.isPulseActive = function(id, callback)
    {
        // There is no need to look for it if the result doesn't matter to the caller...
        if (callback)
        {
            db.loadDatabase(function(loadError)
            {
                ...
                else
                {
                    db.find({"id": id, "type": PULSE_TYPE}, function(err, results)
                    {
                        ...
                        {
                            callback.call(this, null, results[0].active);
                        }
                        else
                        {
                            callback.call(this, "not found", null);
                        }
                    });
                }
            });
